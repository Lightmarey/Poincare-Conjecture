...
\end{remark}
\begin{proof}\uses{thm:short-time-blowup-limit,blowupposcurv,narrows,Harnack}
...
\label{def:strong-epsilon-horn}\uses{def:maximal-extension-to-time-T}
...
\label{def:strong-double-and-capped-horn}\uses{def:strong-epsilon-horn}
...
\label{def:omega-rho}\uses{thm:singular-time-limit-properties,def:strong-epsilon-horn}
For $0<\rho<r_0$, let
...
\label{lem:omega-component-classification}\uses{lem:canonical-neighborhoods-at-time-T,def:omega-rho}
...
\label{rem:possibly-infinitely-many-horns}\uses{def:strong-double-and-capped-horn}
...
\end{theorem}
\begin{proof}\uses{thm:short-time-blowup-limit,thm:splitting-theorem,blowupposcurv,localprod,rm>0,2DGSS}
