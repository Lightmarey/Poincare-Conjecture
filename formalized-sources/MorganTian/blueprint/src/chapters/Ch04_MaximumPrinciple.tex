...
\qquad T\le\frac{n}{2R_\mathrm{min}(0)}.
\]
...
\uses{def:ricci-curvature}\label{firstclaim}
...
\uses{def:convex-subbundle-preserving-vector-field,thm:levi-civita-connection}\label{MP}
...
then ${\mathcal T}(x,0)\in{\mathcal Z}$ on $\overline U$ and boundary values
remain in ${\mathcal Z}$ iff ${\mathcal T}(x,t)\in{\mathcal Z}$ on
\overline U\times[0,T]$.
...
For a compact three-dimensional flow with $\Ric(0)\ge0$, either the flow is
flat or $\Ric>0$ for positive times. In the maximal positive case,
\[
\lim_{t\to T}\frac{\max_{x\in M}\lambda(x,t)}{\min_{x\in M}\nu(x,t)}=1,
\]
$\lambda(x,t)\to\infty$, and rescaling by $\lambda(x,t)$ converges smoothly
to a compact round manifold, hence a spherical space-form.
\end{theorem}
...
\uses{def:ricci-flow,def:curvature-operator}\label{pinch}
