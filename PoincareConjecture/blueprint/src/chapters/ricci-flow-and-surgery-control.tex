...
\item an $\epsilon$-round component containing $x$.
\end{enumerate}
...
\dcref{MT:ch1:1.117,MT:ch1:1.123,MT:ch5:5.10}
\notready
...
\notready
Let a measurable family of minimizing $\mathcal L$-geodesics from a terminal
point remain in a compact smooth domain.  Suppose that at backward time
$\bar\tau$ their regular endpoints contain a measurable set $E$ with
\[
 \operatorname{Vol}(E)\ge V>0,
 \qquad l\le L_0\quad\hbox{on }E.
\]
Suppose also that their terminal ends lie in a parabolic ball
$B(x,T,\rho)$ on which curvature and metric distortion give the density
upper bound $C\rho^{-3}$.  Then
\[
 \operatorname{Vol}B(x,T,\rho)
 \ge C^{-1}(4\pi\bar\tau)^{-3/2}e^{-L_0}V\,\rho^3.
\]
\end{theorem}

\begin{proof}
\uses{lem:reduced-density-monotonicity}
At time $\bar\tau$, the selected geodesics contribute at least
$(4\pi\bar\tau)^{-3/2}e^{-L_0}V$ to reduced volume.  The pointwise
monotonicity in \ref{lem:reduced-density-monotonicity} carries this lower
bound toward the terminal time.  There the same contribution is at most
$C\rho^{-3}\operatorname{Vol}B(x,T,\rho)$.  Comparing the two quantities
and rearranging gives the claimed estimate.  The common compact smooth
domain ensures that no curve crosses a surgery boundary, so there is no
additional boundary contribution.
\end{proof}
...
negative side, let $p$ lie in its middle half, and let $q$ lie anywhere later
on the same axial product line in $N$, with $s(q)>s(p)$.  Then
\[
 (1-\alpha)(s(q)-s(p))
 \le d(z,q)-d(z,p)
 \le(1+\alpha)(s(q)-s(p)).
...
geodesic from $z$ to $q$.  Separation forces its last crossing of the neck
to enter from the negative end.  Once it reaches the fiber through $p$, the
portion already traversed inside the neck has length greater than
$\epsilon^{-1}/100$ after shrinking $\epsilon$.
...
\notready
Assume the stage-$i$ noncollapsing bound on earlier controlled slices.  Under
the hypotheses of \ref{prop:compact-reduced-minimizing-domain}, there is
...
\notready
A three-dimensional \emph{$\kappa$-solution} is a connected Ricci flow
...
\notready
Every orientable two-dimensional $\kappa$-solution is, after a constant
...
point soul.  Every smoothly embedded two-sphere in $M\setminus\{p\}$ has one
...
containing each basepoint has positive sectional curvature.  Then, after
passing to a subsequence, the basepoints eventually have strong
...
$\Omega\subset X(t)$ on which the time-flow identifications converge to an
isometry onto the persistent open subset of $X(T)$.  This isometry extends
continuously to a map
\[
 F\colon X(t)\longrightarrow X(T)
\]
whose restriction to $X(t)\setminus\Omega$ is distance nonincreasing from
$g(t)$ to $g(T)$.
\end{theorem}

\begin{proof}
\uses{def:controlled-ricci-flow-with-surgery,thm:local-neck-surgery,lem:radial-surgery-map-nonexpanding}
On $\Omega$, curvature and all its derivatives remain bounded up to $T$;
the time flow therefore converges smoothly and gives the asserted isometry.
...
converges smoothly.  Hence, after choosing $t$ close enough to $T$, the glued
map is distance nonincreasing on $X(t)\setminus\Omega$.
\end{proof}
