...
\dcref{MT:ch18:18.12}
\notready
Let $g(t)$ be a Ricci flow near $t_0$, and let
...
homotopic.  For each selected postsurgery component $Y$ and every $\eta>0$,
there is, for all $t<T$ sufficiently close to $T$, a map
\[
...
\notready
In a controlled Ricci flow with surgery, only finitely many surgery times use
...
\dcref{MT:ch18:18.9,Hatcher:AT:ch3:prop:3.29,Hatcher:AT:ch4:thm:4.5,Hatcher:AT:ch4:thm:4.32}
\notready
...
cyclic groups.  Then the same is true for every component of every later
time slice.
\end{theorem}
...
\dcref{MT:ch18:18.20}
\notready
Let $X$ be a closed connected three-manifold.  If $\pi_1(X)$ is a
nontrivial free product, or if $\pi_1(X)\cong\mathbb Z$, then
$\pi_2(X)\ne0$.
\end{theorem}

\begin{theorem}[Finite fundamental group and the universal cover]
\label{thm:finite-cover-homotopy-three-sphere}
\dcref{MT:ch18:18.19,Hatcher:Basic3:prop:3.7,Hatcher:Basic3:cor:3.9,
  Hatcher:AT:ch4:thm:4.32}
\notready
...
\dcref{MT:ch18:18.30,MT:ch18:18.32}
\uses{def:loop-family-width}
\notready
Let $g(t)$ be a Ricci flow on a compact three-manifold and let
...
\begin{theorem}[Classical annular Douglas interface]
...
which are freely homotopic and homotopically nontrivial.  Suppose their
spanning-annulus class is nonempty and that there is $\rho>0$ such that, for
every admissible spanning annulus $F$ and every essential simple closed curve
$\alpha$ in its domain,
\[
 L_h(F\circ\alpha)\ge\rho.
\]
Then the annular area infimum is attained by a conformal harmonic annulus.
Its interior branch set is finite, it is real analytic up to its real-analytic
...
homotopy class.  If $\mu(t)$ is the infimum of the areas of spanning annuli,
...
\dcref{MT:ch18:18.59,MT:ch18:18.63}
\uses{def:ramp-curve,thm:annular-douglas-minimizer,
  lem:ramp-annulus-nondegeneration}
\notready
...
\notready
Let $\mathcal X$ be a component path on $[t_0,t_1]$ with
