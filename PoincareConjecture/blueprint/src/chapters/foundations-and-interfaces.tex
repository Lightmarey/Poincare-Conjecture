...
$d(p,q)\ge r_N\epsilon^{-1}/100$, satisfy
\[
 (1-\alpha)r_N|s_N(q)-s_N(p)|
 \le d(p,q)
 \le(1+\alpha)r_N|s_N(q)-s_N(p)|.
\]
In particular, a path exiting both ends has length at least
...
argument applies when a large distance, rather than a large axial
displacement, is assumed, since the spherical diameter is bounded.
