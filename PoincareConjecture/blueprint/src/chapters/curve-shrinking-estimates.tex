...
\begin{theorem}[Parametric well-posedness for immersed curve shrinking]
\label{thm:parametric-curve-shrinking-wellposedness}
\dcref{MT:ch19:19.1}
\notready
Let $P$ be compact, let $0<\alpha<1$, and let $g(t)$ be a smooth Ricci
flow on a compact manifold for $t\in[t_0,t_1]$.  Suppose
$p\mapsto c_{0,p}$ is continuous from $P$ to the space of immersed
$C^{2,\alpha}$ loops and that $|\partial_xc_{0,p}|$ has a positive lower
bound independent of $p$.  Then there is $\tau>0$, independent of $p$, and
a unique family of immersed geometric curve-shrinking flows, up to
time-dependent reparametrization,
\[
 c_p\colon S^1\times[t_0,t_0+\tau]\longrightarrow M
\]
which depends continuously on $p$ in the parabolic
$C^{2,\alpha}$ topology.  Smooth initial data give smooth solutions.  The
family extends up to any finite endpoint $T\le t_1$ at which its curvatures
and immersion constants remain uniformly bounded, and continues across that
endpoint when $T<t_1$.
\end{theorem}
...
\notready
Under the hypotheses of \ref{thm:short-time-curve-curvature-barrier}, there
are constants
...
\]
These constants are uniform over the compact ramp families constructed
above and over $0<\lambda<1$.
\end{theorem}
...
loops.  For every $\zeta>0$ there is a homotopic family of smooth loops
...
\ge\lambda/(2\pi)$.  Thus
\ref{thm:parametric-curve-shrinking-wellposedness}, with parameter space
$S^2$, gives uniqueness and continuous dependence on a common initial
